...
B<riap+tcp>. This scheme is for Riap over direct TCP (see L<Riap::TCP>). The
host (+port) part describes TCP host (+port), and the path part describes path
to code entity. Currently there is no default port so please always specify port
number. Examples:

 riap+tcp://localhost:5000/Text/sprintfn/
 riap+tcp://localhost:5000/Text/sprintfn/sprintfn

B<riap+http>/B<riap+https>. These schemes are for Riap over HTTP/HTTPS (see
L<Riap::HTTP>). The host (+port) part describes HTTP or HTTPS host (+port), and
the path part describes path to code entity. The default port is that of HTTP or
HTTPS. Examples:

 riap+http://gudangapi.com:5000/tax/id/npwp/validate_npwp
 riap+https://gudangapi.com/tax/id/npwp/validate_npwp

Lest there is confusion, with Riap over HTTP/HTTPS there is also the "plain"

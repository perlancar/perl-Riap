...
The protocol can use over any transport protocol, like HTTP or direct TCP.
L<Riap::HTTP> explains how Riap can be used over HTTP.
...
Response is an enveloped result as defined by the L<Rinci::function>
specification.

=item * Request

A Riap request is a mapping of request keys and their values. Server should
return 400 status if required keys are missing, unknown keys are sent, or keys
...
The Riap request is a mapping of request keys and their values. Some recognized
request keys:

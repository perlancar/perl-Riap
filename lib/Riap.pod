...
variables, etc are discovered by client by performing a C<actions> action on a
...
be suffixed with transport, e.g. C<riap+tcp> (see L<Riap::TCP>), while
C<riap+http>/C<riap+https> to refer to Riap over HTTP/HTTPS (see L<Riap::HTTP>).
...
<http>/C<https> URLs to access the services. The URI path for these do not
...
=head2 Action: B<complete_arg_name>

Complete function argument name. Client can actually get metadata using the
C<meta> action and complete argument name itself, but using this action will use
less network bandwidth as the metadata is relatively larger. Additional request
keys:

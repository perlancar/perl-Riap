...
means, for example, L<Serabi>, a Perl server implementation, allows parsing
C<uri> from URI path, and function arguments (as well as other Riap request
keys, using C<-riap-*> syntax) from request variables. For example:
...
Riap maps code entities (packages and functions) to URI's, while REST maps
resources (business "objects", higher-level entities) to URI's. For example:
 ...
 # Riap::HTTP which directly translates URIs from HTTP request to Riap
 /User/get_user?id=123
 /User/modify_user?id=123&name=Lisa
 /User/delete_user?id=123

 # REST (verb + URI)
 GET /user          # list users
 GET /user/123      # get specific user
 PUT /user          # create a new user
 POST /user/123     # modify user
 DELETE /user/123

Which one to choose is arguably just a matter of style. REST-style gives some
consistency in terms of finding resources and standardized verbs, but might be
more cumbersome if you want to introduce more verbs. Riap::HTTP provides
convenience in exporting a set of API modules and functions.

There are Rinci-based tools to create REST-style service. For example, see
L<Serabi> in Perl.

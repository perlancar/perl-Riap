...
This document specifies using HTTP/HTTPS as the transport layer for L<Riap>. For
short, the protocol is called Riap/HTTP.


=head1 DESCRIPTION

Riap/HTTP is designed to be implemented by API service providers. It allows
custom URL layout and some additional features like logging, while utilizing
various features already in HTTP (authentication, encryption using HTTPS, etc.)
...
=head2 Additional information keys must returned by server

These are keys in the information hash returned by the C<info> action.

=over 4

=item * ifmts => ARRAY OF STR

Supported input formats in HTTP request body.

=item * ofmts => ARRAY OF STR

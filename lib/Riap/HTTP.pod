...
Specify output serialization format to use. Server MUST support at least
C<json>. If unspecified, default in most cases should also be C<json>. Server
can default to other format, like L<html> or L<text>, for convenience of certain
clients like GUI browser. Server should detect appropriate default output format
from C<Accept> HTTP request header.

Server can support additional output formats like C<yaml>, C<xml>, or C<phps>
(PHP serialization format). To find out what serializations are supported,
client can perform an C<info> action on a code entity (see below).
...
if it wants, e.g. C<text/yaml> or C<application/vnd.php.serialized>. Client can
also send a multi-part MIME document, where it can send C<args> as well as other
data (like file uploads). It is currently up to the server to support this.

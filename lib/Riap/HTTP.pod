...
If HTTP header C<Content-Range> presents (partial content upload) then the
target function metadata is checked first. If there is a function argument that
has C<partial> property set to 1, then partial argument value will be retrieved
from HTTP body. Special arguments C<-arg_len> and C<-arg_part_start> will be
sent to function with the value from HTTP request header C<Content-Range>.
Special argument C<-arg_part_len> will be sent to function with value from HTTP
request header C<Content-Length>. For example this HTTP request:
